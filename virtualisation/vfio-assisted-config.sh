...
#/>
if [[ -n "${arwrite[@]}" ]]; then
	printf "Modifying line:$(awk -F ':' '{print $1}' <<< "$line") of "$IDFILE" : adding ids ${arwrite[*]}"
...
IDPARAM="vfio-pci.ids="
        ...
        if [[ -n $(grep -o "$IDPARAM") ]]; then
                sed -i "/$BOOTGLOB/ s/$IDPARAM/$IDPARAM${arwrite[$i]},/g"
                continue
        fi
        #if there is neither ids nor vfio-pci.ids=
        if [[ -z $(grep -o "$IDPARAM") ]]; then
        end=$(grep -E "$BOOTGLOB" $IDFILE | grep -oE "[^A-Za-z0-9]$")
        sed -i -E "$BOOTGLOB s/$end/ $IDPARAM${arwrite[$i]}$end/g"
        fi
    fi
